(function(factory){
  ...
  $.fn.monsai = function(options) {
    var args = arguments;
    return this.each(function() {
      var monsai = $(this).data('monsai');
      if (!monsai) {
        monsai = new Monsai(this, options);
        $(this).data('monsai', monsai);
      }
      if (typeof options == 'string') {
        var method = options;
        return monsai[method].apply(monsai, [].slice.call(args, 1));
      }
    });
  };
  $.monsai = {};
  ...
  var Monsai = function(el, options) {
    var self = this;
    options = options || {};
    this.options = $.extend({}, $.monsai.defaults, options);
    this.el = $(el).addClass('monsai').data('monsai', this);
    ...
    if (this.isRootNode()) {
      ...
      this.el.on('click', '.ikon', function(ev) {
  ...
  Monsai.prototype = {
    ...
    update: function() {
      ...
      this.el.children().each(function() {
        ...
        // insert a ikon if it doesn't already exist
        if (item.children().filter('.ikon').length == 0) {
          var ikon = $('<div class="ikon"></div>');
          item.prepend(ikon);
        }
        var subLists = item.children().filter('ol, ul');
        item.toggleClass('has-file', subLists.find('li').length > 0);
        ...
        subLists.each(function() {
          ...
          var exists = !!$(this).data('monsai');
          $(this).monsai(exists ? 'update' : self.options);
        });
      });
    ...
    checkboxPrefix: 'monsai-checkbox-',
    inputIdFor: function(listItem) {
      var id = $(listItem).data('id');
      while (!id || ($('#' + id).length > 0)) {
        id = this.checkboxPrefix + Monsai.uniqueId++;
      }
      return id;
    },
